...
const lookupStar = async() => {
    const instance = await StarNotary.deployed();
    const sid = document.getElementById("starId2").value;
    var sname = await instance.lookUptokenIdToStarInfo.call(sid, { from: account })
    App.setDetails("Star Name is " + sname + ".");
}
...
const App = {
    ...
    setDetails: function(message) {
        const status = document.getElementById('details')
        status.innerHTML = message
    },
